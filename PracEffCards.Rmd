...
```{r, results='hide', include=FALSE, echo=FALSE}
prac_data <- read.csv('Total Practice Stats.csv')
player_row <- prac_data %>% filter(Player == select_player)
header_df <- data.frame(one = c(player_row$image),
                        two = c(paste0(select_player, '<br>Practice Efficiency Card')),
                        three = c(paste0('As of: ', format(Sys.Date(), "%m/%d"))))
header_tbl <- header_df %>%
  gt() %>%
  cols_label(one = '', two = '', three = '') %>%
  text_transform(locations = cells_body(vars(one)), fn = function(x) {web_image(url = x, height = px(60))}) %>%
  ...
  tab_style(style = list(cell_text(weight = "bold", color = "black", size = px(18))), locations = cells_body(vars(two))) %>%
  cols_width(one ~ px(100), two ~ px(240), three ~ px(60))
header_tbl

rank_data <- prac_data[-15,] %>%
  select(Player, FG., eFG., Two., Three., PPS, Rim_att, Mid_att, Three_att, Ast_Tov)
rank_data <- rank_data %>%
  mutate(across(-Player, ~ dense_rank(desc(.)), .names = "{.col}_rk"))
rank_data <- rank_data %>% filter(Player == select_player)
bars_df <- data.frame(Stat = c('FG%', 'eFG%', '2pt%', '3pt%', 'Points Per Shot', 'Rim Att. Rate', 'Mid Att. Rate', '3pt Att. Rate', 'Ast/Tov'),
                      Value = c(round(rank_data$FG.,1), round(rank_data$eFG.,1), round(rank_data$Two.,1), round(rank_data$Three.,1), round(rank_data$PPS,1), round(rank_data$Rim_att,1), round(rank_data$Mid_att,1), round(rank_data$Three_att,1), round(rank_data$Ast_Tov,1)),
                      ptile = c(rank_data$FG._rk, rank_data$eFG._rk, rank_data$Two._rk, rank_data$Three._rk, rank_data$PPS_rk, rank_data$Rim_att_rk, rank_data$Mid_att_rk, rank_data$Three_att_rk, rank_data$Ast_Tov_rk),
                      group = c('Shooting', 'Shooting', 'Shooting', 'Shooting', 'Other', 'Other', 'Other', 'Other', 'Other')
)
bars_tbl <- bars_df %>%
  group_by(group) %>%
  gt() %>%
  cols_label(Stat = md('**Stat**'), Value = md('**Value**'), ptile = md('**Rank Comp. to Team in Practice**')) %>%
  ...
  tab_style(style = cell_text(weight = "bold", align = "center"), locations = cells_body(columns = Stat)) %>%
  tab_style(style = cell_text(decorate = 'underline'), locations = cells_body(columns = ptile)) %>%
  cols_align(align = "center", columns = ptile) %>%
  cols_align(align = "center", columns = Value) %>%
  tab_style(style = list(cell_text(align = "center", style = "italic"), cell_fill(color = "gray90")), locations = cells_row_groups()) %>%
  cols_width(Stat ~ px(150), Value ~ px(70), ptile ~ px(170)) %>%
  tab_footnote(footnote = md("*Points Per Shot - not including free throws*"), locations = cells_body(columns = Stat, rows = 5)) %>%
  tab_footnote(footnote = md("*per 100*"), locations = cells_body(columns = Stat, rows = 6:9)) %>%
  tab_style(style = list(cell_text(align = 'center')), locations = cells_footnotes())
bars_tbl
...
player_shots <- shotchart_data_list[[select_player]]
combined_df <- player_shots %>%
...
combined_df <- combined_df %>%
  mutate(
    Rim_color = custom_color_scale(Rim.),
    Paint_color = custom_color_scale(Paint.),
    MidLC_color = custom_color_scale(MidLC.),
    MidLW_color = custom_color_scale(MidLW.),
    MidRW_color = custom_color_scale(MidRW.),
    MidRC_color = custom_color_scale(MidRC.),
    ThreeLC_color = custom_color_scale(ThreeLC.),
    ThreeLW_color = custom_color_scale(ThreeLW.),
    ThreeTop_color = custom_color_scale(ThreeTop.),
    ThreeRW_color = custom_color_scale(ThreeRW.),
    ThreeRC_color = custom_color_scale(ThreeRC.)
  )
court <- ggplot() +
  # Outline court
  ...
  geom_rect(aes(ymin = 0, ymax = -19, xmin = 8, xmax = 8), color = 'black', fill = NA) +
  # paint color
  geom_rect(aes(ymin = -19, ymax = 0, xmin = -8, xmax = 8), fill = combined_df$Paint_color, color = NA, alpha = 0.5) +
  # rim color
  geom_circle(aes(y0 = -4.5, x0 = 0, r = 4.5), fill = combined_df$Rim_color, color = NA, alpha = 1) +
  # mid left corner color
  geom_polygon(aes(x = c(-24.7, -8, -8, -25.76, -25.76),
                   y = c(-11, -11, 0, 0, -8)), fill = combined_df$MidLC_color, alpha = 0.5) +
  # mid right corner color
  geom_polygon(aes(x = c(24.7, 8, 8, 25.76, 25.76),
                   y = c(-11, -11, 0, 0, -8)), fill = combined_df$MidRC_color, alpha = 0.5) +
  # mid left wing color
  geom_polygon(aes(x = c(-8, 0, 0, -4, -8),
                   y = c(-19, -19, -27, -26.8, -25.8)), fill = combined_df$MidLW_color, alpha = 0.5) +
  geom_polygon(aes(x = c(-24.7, -8, -8, -10, -11, -12, -13, -13.5, -14, -14.5, -15, -16, -16.5, -17, -18, -19, -20, -21, -22),
                   y = c(-11, -11, -25.8, -25, -24.7, -24, -23.8, -23.4, -23, -22.8, -22.2, -21.8, -21.5, -21, -20.2, -19.3, -18.3, -17.2, -16)), fill = combined_df$MidLW_color, alpha = 0.5) +
  # mid right wing color
  geom_polygon(aes(x = c(8, 0, 0, 4, 8),
                   y = c(-19, -19, -27, -26.8, -25.8)), fill = combined_df$MidRW_color, alpha = 0.5) +
  geom_polygon(aes(x = c(24.7, 8, 8, 10, 11, 12, 13, 13.5, 14, 14.5, 15, 16, 16.5, 17, 18, 19, 20, 21, 22),
                   y = c(-11, -11, -25.8, -25, -24.7, -24, -23.8, -23.4, -23, -22.8, -22.2, -21.8, -21.5, -21, -20.2, -19.3, -18.3, -17.2, -16)), fill = combined_df$MidRW_color, alpha = 0.5) +
  # three left corner color
  geom_polygon(aes(x = c(-30, -25.8, -25.8, -24.5, -30),
                   y = c(0, 0, -8, -11, -11)), fill = combined_df$ThreeLC_color, alpha = 0.5) +
  # three right corner color
  geom_polygon(aes(x = c(30, 25.8, 25.8, 24.5, 30),
                   y = c(0, 0, -8, -11, -11)), fill = combined_df$ThreeRC_color, alpha = 0.5) +
  # three left wing color
  geom_polygon(aes(x = c(-30, -30, -24.5, -23, -22, -21, -20, -19, -13, -13),
                   y = c(-40, -11, -11, -14, -15.5, -17.2, -18.3, -19.3, -23.8, -40)), fill = combined_df$ThreeLW_color, alpha = 0.5) +
  # three right wing color
  geom_polygon(aes(x = c(30, 30, 24.5, 23, 22, 21, 20, 19, 13, 13),
                   y = c(-40, -11, -11, -14, -15.5, -17.2, -18.3, -19.3, -23.8, -40)), fill = combined_df$ThreeRW_color, alpha = 0.5) +
  # three top key color
  geom_polygon(aes(x = c(13, -13, -13, -10, -8, -5, -3, 0, 3, 5, 8, 10, 13),
                   y = c(-40, -40, -23.8, -25, -25.8, -26.3, -26.7, -27, -26.7, -26.3, -25.8, -25, -23.8)), fill = combined_df$ThreeTop_color, alpha = 0.5) +
  # rim text
